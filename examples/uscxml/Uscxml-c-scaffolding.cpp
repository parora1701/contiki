...
//#undef CONTIKIHELLOWORLDINVOKER
//#define WITH_CONTIKI_SETTINGS_EXAMPLE_INVOKER
//#define WITH_CONTIKI_CALC_EXAMPLE_INVOKER
//#define WITH_CONTIKI_EXAMPLE_POWERTRACE_INVOKER
...
//#include "/Users/sradomski/Documents/TK/Code/uscxml/build/cli/test/gen/c/ecma/test144.scxml.machine.c"
#include "input/test144.c"
#endif
...
#ifdef WITH_CONTIKI_EXAMPLE_POWERTRACE_INVOKER
#include "uscxml/plugins/invoker/contiki/contiki_example-powertrace_Invoker.h"
#endif
#ifdef WITH_CONTIKI_SETTINGS_EXAMPLE_INVOKER
#include "uscxml/plugins/invoker/contiki/contiki_settings-example_Invoker.h"
#endif

#ifdef WITH_CONTIKI_CALC_EXAMPLE_INVOKER
#include "uscxml/plugins/invoker/contiki/contiki_calc_Invoker.h"
#endif


#ifdef CONTIKIHELLOWORLDINVOKER
#include "uscxml/plugins/invoker/contiki/contiki_hello-world_Invoker.h"
#endif
...
//        finalize();
	...
	static int invoke(const uscxml_ctx* ctx, const uscxml_state* s, const uscxml_elem_invoke* invocation, unsigned char uninvoke) {
		...
		if (uninvoke) {
			...
			if (invocation->machine != NULL) {
				...
				if (invocation->id!=NULL) {
					invokeEvent.invokeid = invocation->id;
				}
				else {
					invokeEvent.invokeid = invocation->sourcename + '.' + uscxml::UUID::getUUID();
					if (invocation->idlocation!=NULL) {
						USER_DATA(ctx)->dataModel.assign(invocation->idlocation, Data(invokeEvent.invokeid, Data::VERBATIM), std::map<std::string, std::string>());
					}
				}

				// namelist
				if (invocation->namelist != NULL) {
					processNameLists(ctx, invokeEvent.namelist, invocation->namelist);
				}

				// params
				if (invocation->params != NULL) {
					processParam(ctx, invokeEvent.params, invocation->params);
				}

				// content
				if (invocation->content != NULL) {
					if (isNumeric(invocation->content, 10)) {
						// test 529
						invokeEvent.data = Data(strTo<double>(invocation->content), Data::INTERPRETED);
					}
					else {
						invokeEvent.data = Data(invocation->content, Data::VERBATIM);
					}
				}             
				...
				Event done;
				done.invokeid =  invokeEvent.invokeid;
				
				done.name = "done.invoke." + (std::string)invokeEvent.invokeid;
				USER_DATA(ctx)->eq.push_back(done);
	...
	static void processNameLists(const uscxml_ctx* ctx,std::map<std::string, Data>& nameMap, const char* nameList) {
			nameMap[nameList] = USER_DATA(ctx)->dataModel.evalAsData(nameList);
	}

	static void processParam(const uscxml_ctx* ctx, std::multimap<std::string, Data>& paramMap, const uscxml_elem_param* invokeParam) {
		try {
			const uscxml_elem_param* param = invokeParam;
			while (param && USCXML_ELEM_PARAM_IS_SET(param)) {
				...
				paramMap.insert(std::make_pair(param->name, paramValue));
				param++;
			}
		}
		catch (Event e) {
...
//    HTTPServer::getInstance(3453, 3454, NULL);
    ...
    //Factory::getInstance()->registerInvoker(new DirMonInvoker());
#ifdef CONTIKIHELLOWORLDINVOKER
	Factory::getInstance()->registerInvoker(new contiki_hello_world_Invoker());
#endif // DEBUG


#ifdef WITH_CONTIKI_SETTINGS_EXAMPLE_INVOKER
	Factory::getInstance()->registerInvoker(new contiki_settings_example_Invoker());
#endif

#ifdef WITH_CONTIKI_EXAMPLE_POWERTRACE_INVOKER
	Factory::getInstance()->registerInvoker(new contiki_example_powertrace_Invoker());
#endif

#ifdef WITH_CONTIKI_CALC_EXAMPLE_INVOKER
	Factory::getInstance()->registerInvoker(new contiki_calc_Invoker());
#endif
